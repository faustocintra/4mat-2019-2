...
export class CursoService {
  ...
  /* Um objeto da classe HttpClient é injetado
    como dependência da classe CursoService.
    Isso é chamado INJEÇÃO DE DEPENDÊNCIA */
  constructor(private http: HttpClient) { 
    
  }

  private entryPoint : string = 'curso';
  ...
  novo(curso: any) {
    return this.http.post(env.apiBaseUri + this.entryPoint, curso).toPromise();
  }
  ...
  atualizar(curso: any) {
    return this.http.put(env.apiBaseUri + this.entryPoint, curso).toPromise();
  }
